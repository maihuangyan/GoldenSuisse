<template lang="pug">
.invest
  section.invest__title(
    data-anim="fadeIn",
    data-anim-delay="0",
    :style="{ opacity: this.$store.state.isMobile ? 1 : 0, transform: this.$store.state.isMobile ? 'translate(0, 0)' : 'translate(0, 10%)' }"
  )
    | Invest in Gold
    br
    | and Silver
  section.invest__about(
    data-anim="fadeIn",
    data-anim-delay="0",
    :style="{ opacity: this.$store.state.isMobile ? 1 : 0, transform: this.$store.state.isMobile ? 'translate(0, 0)' : 'translate(0, 5%)' }"
  )
    | Coins containing gold appeared around 800 B.C., and the first pure gold coins were struck during the rein of King Croesus of Lydia about 300 years later.
  section.invest__about3(
    data-anim="fadeIn",
    data-anim-delay="0",
    :style="{ opacity: this.$store.state.isMobile ? 1 : 0, transform: this.$store.state.isMobile ? 'translate(0, 0)' : 'translate(0, 5%)' }"
  )
    | Throughout the centuries, people have continued to hold gold for various reasons. Below are eight reasons to own gold today.
  section.invest__about2(
    data-anim="fadeIn",
    data-anim-delay="0",
    :style="{ opacity: this.$store.state.isMobile ? 1 : 0, transform: this.$store.state.isMobile ? 'translate(0, 0)' : 'translate(0, 5%)' }"
  )
    | Gold is respected throughout the world for its value and rich history, which has been interwoven into cultures for thousands of years.
  section.invest__about4(
    data-anim="fadeIn",
    data-anim-delay="0",
    :style="{ opacity: this.$store.state.isMobile ? 1 : 0, transform: this.$store.state.isMobile ? 'translate(0, 0)' : 'translate(0, 5%)' }"
  )
    | A History of Holding Its Value
  section.invest__about5(
    data-anim="fadeIn",
    data-anim-delay="0",
    :style="{ opacity: this.$store.state.isMobile ? 1 : 0, transform: this.$store.state.isMobile ? 'translate(0, 0)' : 'translate(0, 5%)' }"
  )
    | Unlike paper currency, coins or other assets, gold has maintained its value throughout the ages. People see gold as a way to pass on and preserve their wealth from one generation to the next.
  section.invest__about6(
    data-anim="fadeIn",
    data-anim-delay="0",
    :style="{ opacity: this.$store.state.isMobile ? 1 : 0, transform: this.$store.state.isMobile ? 'translate(0, 0)' : 'translate(0, 5%)' }"
  )
    | Weakness of the U.S. Dollar
  section.invest__about7(
    data-anim="fadeIn",
    data-anim-delay="0",
    :style="{ opacity: this.$store.state.isMobile ? 1 : 0, transform: this.$store.state.isMobile ? 'translate(0, 0)' : 'translate(0, 5%)' }"
  )
    | Although the U.S. dollar is one of the world’s most important reserve currencies, when the value of the dollar falls against other currencies as it did between 1998 and 2008, this often prompts people to flock to the security of gold, which raises gold prices . The price of gold nearly tripled between 1998 and 2008, reaching the $1,000-an-ounce milestone in early 2008 and nearly doubling between 2008 and 2012, hitting around the $1800-$1900 mark. The decline in the U.S. dollar occurred for a number of reasons, including the country’s large budget and trade deficits and a large increase in the money supply.
  section.invest__about8(
    data-anim="fadeIn",
    data-anim-delay="0",
    :style="{ opacity: this.$store.state.isMobile ? 1 : 0, transform: this.$store.state.isMobile ? 'translate(0, 0)' : 'translate(0, 5%)' }"
  )
    | Inflation
  section.invest__about9(
    data-anim="fadeIn",
    data-anim-delay="0",
    :style="{ opacity: this.$store.state.isMobile ? 1 : 0, transform: this.$store.state.isMobile ? 'translate(0, 0)' : 'translate(0, 5%)' }"
  )
    | Gold has historically been an excellent hedge against inflation, because its price tends to rise when the cost of living increases. Over the past 50 years investors have seen gold prices soar and the stock market plunge during high-inflation years.
  section.invest__about10(
    data-anim="fadeIn",
    data-anim-delay="0",
    :style="{ opacity: this.$store.state.isMobile ? 1 : 0, transform: this.$store.state.isMobile ? 'translate(0, 0)' : 'translate(0, 5%)' }"
  )
    | Deflation
  section.invest__about11(
    data-anim="fadeIn",
    data-anim-delay="0",
    :style="{ opacity: this.$store.state.isMobile ? 1 : 0, transform: this.$store.state.isMobile ? 'translate(0, 0)' : 'translate(0, 5%)' }"
  )
    | Deflation, a period in which prices decrease, business activity slows and the economy is burdened by excessive debt, has not been seen globally since the Great Depression of the 1930s. During that time, the relative purchasing power of gold soared while other prices dropped sharply.
  section.invest__about12(
    data-anim="fadeIn",
    data-anim-delay="0",
    :style="{ opacity: this.$store.state.isMobile ? 1 : 0, transform: this.$store.state.isMobile ? 'translate(0, 0)' : 'translate(0, 5%)' }"
  )
    | Geopolitical Uncertainty
  section.invest__about13(
    data-anim="fadeIn",
    data-anim-delay="0",
    :style="{ opacity: this.$store.state.isMobile ? 1 : 0, transform: this.$store.state.isMobile ? 'translate(0, 0)' : 'translate(0, 5%)' }"
  )
    | Gold retains its value not only in times of financial uncertainty, but in times of geopolitical uncertainty. It is often called the “crisis commodity,” because people flee to its relative safety when world tensions rise; during such times, it often outperforms other investments. For example, gold prices experienced some major price movements this year in response to the crisis occurring in the European Union. Its price often rises the most when confidence in governments is low.
  section.invest__about14(
    data-anim="fadeIn",
    data-anim-delay="0",
    :style="{ opacity: this.$store.state.isMobile ? 1 : 0, transform: this.$store.state.isMobile ? 'translate(0, 0)' : 'translate(0, 5%)' }"
  )
    | Supply Constraints
  section.invest__about15(
    data-anim="fadeIn",
    data-anim-delay="0",
    :style="{ opacity: this.$store.state.isMobile ? 1 : 0, transform: this.$store.state.isMobile ? 'translate(0, 0)' : 'translate(0, 5%)' }"
  )
    | Much of the supply of gold in the market since the 1990s has come from sales of gold bullion from the vaults of global central banks. This selling by global central banks slowed greatly in 2008. At the same time, production of new gold from mines had been declining since 2000.
    br
    | It can take from five to 10 years to bring a new mine into production. As a general rule, reduction in the supply of gold increases gold prices.
  section.invest__about16(
    data-anim="fadeIn",
    data-anim-delay="0",
    :style="{ opacity: this.$store.state.isMobile ? 1 : 0, transform: this.$store.state.isMobile ? 'translate(0, 0)' : 'translate(0, 5%)' }"
  )
    | Increasing Demand
  section.invest__about17(
    data-anim="fadeIn",
    data-anim-delay="0",
    :style="{ opacity: this.$store.state.isMobile ? 1 : 0, transform: this.$store.state.isMobile ? 'translate(0, 0)' : 'translate(0, 5%)' }"
  )
    | In previous years, increased wealth of emerging market economies boosted demand for gold. In many of these countries, gold is intertwined into the culture. India is one of the largest gold-consuming nations in the world; it has many uses there, including jewellery. As such, the Indian wedding season in October is traditionally the time of the year that sees the highest global demand for gold (though it has taken a tumble in 2012.) In China, where gold bars are a traditional form of saving, the demand for gold has been steadfast.
  section.invest__about18(
    data-anim="fadeIn",
    data-anim-delay="0",
    :style="{ opacity: this.$store.state.isMobile ? 1 : 0, transform: this.$store.state.isMobile ? 'translate(0, 0)' : 'translate(0, 5%)' }"
  )
    | Demand for gold has also grown among investors.
  section.invest__about19(
    data-anim="fadeIn",
    data-anim-delay="0",
    :style="{ opacity: this.$store.state.isMobile ? 1 : 0, transform: this.$store.state.isMobile ? 'translate(0, 0)' : 'translate(0, 5%)' }"
  )
    | Many are beginning to see commodities, particularly gold, as an investment class into which funds should be allocated. In fact, SPDR Gold Trust, became one of the largest ETFs in the U.S., as well as one of the world’s largest holders of gold bullion in 2008, only four years after its inception.
  section.invest__about20(
    data-anim="fadeIn",
    data-anim-delay="0",
    :style="{ opacity: this.$store.state.isMobile ? 1 : 0, transform: this.$store.state.isMobile ? 'translate(0, 0)' : 'translate(0, 5%)' }"
  )
    | Portfolio Diversification
  section.invest__about21(
    data-anim="fadeIn",
    data-anim-delay="0",
    :style="{ opacity: this.$store.state.isMobile ? 1 : 0, transform: this.$store.state.isMobile ? 'translate(0, 0)' : 'translate(0, 5%)' }"
  )
    | The key to diversification is finding investments that are not closely correlated to one another; gold has historically had a negative correlation to stocks and other financial instruments.
  section.invest__about22(
    data-anim="fadeIn",
    data-anim-delay="0",
    :style="{ opacity: this.$store.state.isMobile ? 1 : 0, transform: this.$store.state.isMobile ? 'translate(0, 0)' : 'translate(0, 5%)' }"
  )
    | The 1970s was great for gold, but terrible for stocks.
  section.invest__about23(
    data-anim="fadeIn",
    data-anim-delay="0",
    :style="{ opacity: this.$store.state.isMobile ? 1 : 0, transform: this.$store.state.isMobile ? 'translate(0, 0)' : 'translate(0, 5%)' }"
  )
    | The 1980s and 1990s were wonderful for stocks, but horrible for gold. 2008 saw stocks drop substantially as consumers migrated to gold. Properly diversified investors combine gold with stocks and bonds in a portfolio to reduce the overall volatility and risk.
  section.invest__about24(
    data-anim="fadeIn",
    data-anim-delay="0",
    :style="{ opacity: this.$store.state.isMobile ? 1 : 0, transform: this.$store.state.isMobile ? 'translate(0, 0)' : 'translate(0, 5%)' }"
  )
    | The Bottom Line
  section.invest__about25(
    data-anim="fadeIn",
    data-anim-delay="0",
    :style="{ opacity: this.$store.state.isMobile ? 1 : 0, transform: this.$store.state.isMobile ? 'translate(0, 0)' : 'translate(0, 5%)' }"
  )
    | Gold should be an important part of a diversified investment portfolio because its price increases in response to events that cause the value of paper investments, such as stocks and bonds, to decline. Although the price of gold can be volatile in the short term, it has always maintained its value over the long term. Through the years, it has served as a hedge against inflation and the erosion of major currencies, and thus is an investment well worth considering.

  section.invest__adv(
    data-anim="fadeIn",
    data-anim-delay=".3",
    :style="{ opacity: this.$store.state.isMobile ? 1 : 0 }"
  )
    .invest__adv_title Apply for a
    .invest__adv_desc Private Swiss vault account
  section.invest__createAcc
    .invest__createAcc_container
      FlowButtonDark(label="Become a Client", path="/account/register")
</template>

<script>
export default {
  name: "Invest",
  head: {
    title: "Golden Suisse - Invest in Gold and Silver",
  },
  layout: "white",
  components: {
    FlowButtonDark: () => import("@/components/FlowButtonDark"),
    CardType: () => import("@/components/GoldCard/CardType"),
    LogoSvg: () => import("@/components/LogoSvg"),
    WhiteList: () => import("@/components/WhiteList"),
  },
  data() {
    return {
      list: [
        "heck your card EUR balance and transactions",
        "Load your card in seconds, by selling your vaulted gold coins at market prices",
        "Activate and Lock Your Account",
        "Check your PIN Code",
        "Check your Card Details",
      ],
    };
  },
  methods: {
    redirect(path) {},
  },
};
</script>

<style lang="scss" scoped>
@import "@/assets/_variables.scss";

.children {
  &__button {
    position: fixed;
    bottom: 0px;
    width: 30%;
    height: 100%;
    z-index: 33;
    cursor: pointer;
    &_left {
      left: 0px;
    }
    &_right {
      right: 0px;
    }
  }
}

.invest {
  display: grid;
  grid-template-areas:
    "title title"
    "about2 about"
    "about2 about3"
    "about4 about5"
    "about6 about7"
    "about8 about9"
    "about10 about11"
    "about12 about13"
    "about14 about15"
    "about16 about17"
    "about18 about19"
    "about20 about21"
    "about22 about23"
    "about24 about25"
    "logo logo"
    "cardList cardList"
    ". mobiles"
    "adv adv"
    "createAcc createAcc"
    "footer footer";
  grid-template-columns: repeat(2, 50%);
  padding: 11% 3.75% 3.6% 3.75%;
  padding-bottom: 0 !important;
  &__title {
    grid-area: title;
    font-family: Arial;
    font-style: normal;
    font-weight: normal;
    font-size: 180px;
    line-height: 89%;
    display: flex;
    align-items: center;
    letter-spacing: -0.04em;
    color: $black;
    padding: 5% 12.5% 5% 0;
  }

  &__about3 {
    grid-area: about3;
  }

  &__about4 {
    grid-area: about4;
  }

  &__about5 {
    grid-area: about5;
  }

  &__about6 {
    grid-area: about6;
  }

  &__about7 {
    grid-area: about7;
  }

  &__about8 {
    grid-area: about8;
  }

  &__about9 {
    grid-area: about9;
  }

  &__about10 {
    grid-area: about10;
  }

  &__about11 {
    grid-area: about11;
  }

  &__about12 {
    grid-area: about12;
  }

  &__about13 {
    grid-area: about13;
  }

  &__about14 {
    grid-area: about14;
  }

  &__about15 {
    grid-area: about15;
  }

  &__about16 {
    grid-area: about16;
  }

  &__about17 {
    grid-area: about17;
  }

  &__about18 {
    grid-area: about18;
  }

  &__about19 {
    grid-area: about19;
  }

  &__about20 {
    grid-area: about20;
  }

  &__about21 {
    grid-area: about21;
  }

  &__about22 {
    grid-area: about22;
  }

  &__about23 {
    grid-area: about23;
  }

  &__about24 {
    grid-area: about24;
  }

  &__about25 {
    grid-area: about25;
  }

  &__about {
    grid-area: about;
  }

  &__about,
  &__about3,
  &__about4,
  &__about5,
  &__about6,
  &__about7,
  &__about8,
  &__about9,
  &__about10,
  &__about11,
  &__about12,
  &__about13,
  &__about14,
  &__about15,
  &__about16,
  &__about17,
  &__about18,
  &__about19,
  &__about20,
  &__about21,
  &__about22,
  &__about23,
  &__about24,
  &__about25 {
    font-family: Arial;
    font-style: normal;
    font-weight: normal;
    font-size: 40px;
    line-height: 150%;
    letter-spacing: -0.02em;
    color: $black;
    padding: 5% 0;
    padding-right: 17.6%;
  }

  &__about2 {
    grid-area: about2;
    font-family: Arial;
    font-style: normal;
    font-weight: normal;
    font-size: 40px;
    line-height: 150%;
    letter-spacing: -0.02em;
    color: $black;
    padding: 5% 0;
    padding-right: 35%;
  }

  &__mobiles {
    grid-area: mobiles;
    width: 50%;
    padding-right: 10%;
    display: inline-flex;

    &_ios {
      width: 142px;
      height: 50px;
      margin-right: 7%;
    }

    &_andriod {
      width: 142px;
      height: 50px;
    }
  }

  &__logo {
    grid-area: logo;
    display: flex;
    justify-content: center;
    padding: 10% 0 13.8% 0;

    &_large {
      transform: scale(2);
    }
  }

  &__cardList {
    grid-area: cardList;
  }

  &__adv {
    grid-area: adv;
    display: block;
    padding-top: 4%;
    text-align: center;
    height: 100%;
    padding: 10.3% 0 3% 0;
    font-style: normal;
    font-weight: bold;
    font-size: 14px;
    color: $darkGrey;
    line-height: 143%;
    letter-spacing: -0.04em;

    &_title {
      width: 100%;
    }

    &_desc {
      width: 100%;
    }
  }

  &__createAcc {
    grid-area: createAcc;
    display: flex;
    justify-content: center;
    padding: 1.3% 0 11% 0;
    padding-bottom: 0 !important;
    &_container {
      width: 370px;
    }
  }

  &__footer {
    grid-area: footer;
    font-family: Arial;
    font-style: normal;
    font-weight: normal;
    font-size: 14px;
    line-height: 214%;
    display: flex;
    justify-content: center;
    letter-spacing: -0.02em;
    color: $darkGrey;
  }
}

@media (max-width: 1023px) {
  .invest {
    grid-template-areas:
      "title"
      "about2"
      "about"
      "about3"
      "about4" "about5"
      "about6" "about7"
      "about8" "about9"
      "about10" "about11"
      "about12" "about13"
      "about14" "about15"
      "about16" "about17"
      "about18" "about19"
      "about20" "about21"
      "about22" "about23"
      "about24" "about25"
      "logo"
      "cardList"
      "mobiles"
      "adv"
      "createAcc"
      "footer";
    grid-template-columns: 100%;
    padding: 21% 5.4% 16% 5.4%;

    &__about,
    &__about2,
    &__about3,
    &__about4,
    &__about5,
    &__about6,
    &__about7,
    &__about8,
    &__about9,
    &__about10,
    &__about11,
    &__about12,
    &__about13,
    &__about14,
    &__about15,
    &__about16,
    &__about17,
    &__about18,
    &__about19,
    &__about20,
    &__about21,
    &__about22,
    &__about23,
    &__about24,
    &__about25 {
      font-family: Arial;
      font-style: normal;
      font-weight: normal;
      font-size: 24px;
      line-height: 150%;
      letter-spacing: -0.02em;
      color: $black;
      padding: 5% 0;
      padding-right: 0;
    }

    &__title {
      font-size: 48px;
      line-height: 104%;
      letter-spacing: -0.04em;
      padding: 5% 0 10% 0;
    }

    &__logo {
      padding: 10% 0 45% 0;

      &_large {
        transform: scale(1);
      }
    }

    &__adv {
      padding: 14% 0 14% 0;
    }

    &__createAcc {
      padding: 5% 0 40% 0;
    }
  }
}

@media (max-width: 410px) {
  .invest {
    &__createAcc {
      &_container {
        width: 100%;
      }
    }
  }
}
</style>
